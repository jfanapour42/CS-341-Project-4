...
module lexer =
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               // could be real literal:
               if lookahead = '.' then
                 let (rLiteral, nextLook) = collectIntLiteral (nextChar input) input (literal + (string lookahead))
                 lexer nextLook input (("real_literal:" + rLiteral) :: tokens)
               else
